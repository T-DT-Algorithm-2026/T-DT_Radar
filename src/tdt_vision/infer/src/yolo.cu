...
const int NUM_BOX_ELEMENT = 15;  // left, top, right, bottom, confidence,
                                 // class, keepflag, row_index(output)
...
public:
              ...
              float confidence_threshold, float nms_threshold)
    {
        ...
        if (type == Type::V5 || type == Type::V3 || type == Type::V7) {
            ...
            // normalize_ = Norm::mean_std(mean, std, 1/255.0f,
            // ChannelType::SwapRB);
                                      ...
                                      void* stream = nullptr) override
    {
        ...
        auto time_used3d3 =
            std::chrono::duration_cast<std::chrono::duration<double>>(a3d3 -
                                                                      a3d2);
        //    INFO("decode_kernel_invoker time: %f", time_used3d3.count() *
        //    1000);
        ...
        auto time_used3d4 =
            std::chrono::duration_cast<std::chrono::duration<double>>(a4 -
                                                                      a3d4);
        //    INFO("copy output_boxarray_ time: %f", time_used3d4.count() *
        //    1000);
        auto time_used4 =
            std::chrono::duration_cast<std::chrono::duration<double>>(a4 -
                                                                      a3);
        ...
        auto time_used6 =
            std::chrono::duration_cast<std::chrono::duration<double>>(a6 -
                                                                      a5);
        //    INFO("decode_single_mask time: %f", time_used6.count() *
        //    1000);
