...
static string file_name(const string& path, bool include_suffix)
{
    ...
    return include_suffix
               ? path.substr(p)
               : path.substr(p, (path.rfind('.') <= p ? path.size()
                                                      : path.rfind('.')) -
                                    p);
}

// __global__ void transposeKernel(float* input, float* output, int rows,
// int cols) {
...
// void transpose(float* input, float* output, int rows, int cols, void*
// stream) {
//     dim3 blockSize(16, 16);
//     dim3 gridSize((cols + blockSize.x - 1) / blockSize.x, (rows +
//     blockSize.y - 1) / blockSize.y); transposeKernel<<<gridSize,
//     blockSize, 0, (cudaStream_t)stream>>>(input, output, rows, cols);
// }
...
void __log_func(const char* file, int line, const char* fmt, ...)
{
    ...
    int n = snprintf(buffer, sizeof(buffer), "[%s:%d]: ", filename.c_str(),
                     line);
...
float Timer::stop(const char* prefix, bool print)
{
    ...
    float latency = 0;
    checkRuntime(cudaEventElapsedTime(&latency, (cudaEvent_t)start_,
                                      (cudaEvent_t)stop_));
...
public:
    ...
    bool load(const string& file)
    {
        ...
        if (data.empty()) {
            INFO("An empty file has been loaded. Please confirm your file "
                 "path: %s",
                 file.c_str());
            return false;
        }
                         ...
                         void* input_consum_event) override
    {
        ...
        for (int i = 0; i < nbBindings; ++i) {
            if (!context->setTensorAddress(engine->getIOTensorName(i),
                                           bindings[i])) {
                return false;
            }
        }
        ...
        if (input_consum_event) {
            context->setInputConsumedEvent(
                static_cast<cudaEvent_t>(input_consum_event));
        }
    ...
    virtual std::vector<int> run_dims(int ibinding) override
    {
        ...
        auto dim =
            this->context_->engine_->getTensorShape(tensorName.c_str());
    ...
    virtual bool is_input(int ibinding) override
    {
        const char* tensorName =
            this->context_->engine_->getIOTensorName(ibinding);
        TensorIOMode ioMode =
            this->context_->engine_->getTensorIOMode(tensorName);
    ...
    virtual DType dtype(int ibinding) override
    {
        const char* tensorName =
            this->context_->engine_->getIOTensorName(ibinding);
        DataType dataType =
            this->context_->engine_->getTensorDataType(tensorName);
    ...
    virtual bool has_dynamic_dim() override
    {
        ...
        for (int i = 0; i < numIOTensors; ++i) {
            const char* tensorName =
                this->context_->engine_->getIOTensorName(i);
    ...
    virtual void print() override
    {
        ...
        for (int i = 0; i < num_input; ++i) {
            ...
            INFO("\t%d.%s : shape {%s}", i, name,
                 format_shape(dim).c_str());
        }
